module.exports.run = async (bot, message, args, lang, v, logger) => {
    ...
    if (v.beepBot.settings[guildid].adminroles.length > 0) {
        var adminroles = new String
        for(i = 0; i < v.beepBot.settings[guildid].adminroles.length; i++) { //< and not <= because i is one lower than length
            if (i < 1) adminroles += message.guild.roles.cache.get(v.beepBot.settings[guildid].adminroles[i]).name
                else adminroles += ", " + message.guild.roles.cache.get(v.beepBot.settings[guildid].adminroles[i]).name }
    } else { var adminroles = none }

    if (v.beepBot.settings[guildid].moderatorroles.length > 0) {
        var moderatorroles = new String
        for(i = 0; i < v.beepBot.settings[guildid].moderatorroles.length; i++) { //< and not <= because i is one lower than length
            if (i < 1) moderatorroles += message.guild.roles.cache.get(v.beepBot.settings[guildid].moderatorroles[i]).name
                else moderatorroles += ", " + message.guild.roles.cache.get(v.beepBot.settings[guildid].moderatorroles[i]).name }  
    } else { var moderatorroles = none }

    if (v.beepBot.settings[guildid].memberaddroles.length > 0) {
        var memberaddroles = new String
        for(i = 0; i < v.beepBot.settings[guildid].memberaddroles.length; i++) { //< and not <= because i is one lower than length
            if (i < 1) memberaddroles += message.guild.roles.cache.get(v.beepBot.settings[guildid].memberaddroles[i]).name
                else memberaddroles += ", " + message.guild.roles.cache.get(v.beepBot.settings[guildid].memberaddroles[i]).name }  
    } else { var memberaddroles = none }

    //systemchannel
    if (v.beepBot.settings[guildid].systemchannel !== null) var systemchannel = "#" + message.guild.channels.cache.get(v.beepBot.settings[guildid].systemchannel).name
    ...
    //greetmsg & byemsg
    if (v.beepBot.settings[guildid].greetmsg === null) var greetmsg = none
        else var greetmsg = v.beepBot.settings[guildid].greetmsg

    if (v.beepBot.settings[guildid].byemsg === null) var byemsg = none
        else var byemsg = v.beepBot.settings[guildid].byemsg
    ...
    switch(args[0].toLowerCase()) {
        case "help":
            var PREFIX = v.beepBot.settings[guildid].prefix
...
${lf.helpadvice}
        ...
        case "prefix":
            ...
            if (message.guild.members.cache.get(bot.user.id).nickname === null) var nickname = bot.user.username 
                else var nickname = message.guild.members.cache.get(bot.user.id).nickname.replace(` [${v.beepBot.settings[guildid].prefix}]`, "")

            v.beepBot.settings[guildid].prefix = args[1];
        ...
        case "lang":
            ...
            if (!Object.keys(v.langObj).includes(args[1].toLowerCase())) {
                ...
                v.beepBot.settings[guildid].lang = args[1].toLowerCase();
                writenewsettings();
        ...
        case "adminrole":
            ...
            switch(args[1].toLowerCase()) {
                case "add":
                    var roleid = roleid();
                    if (v.beepBot.settings[guildid].adminroles.includes(roleid)) return message.channel.send(lf.rolealreadyadded)
                    v.beepBot.settings[guildid].adminroles.push(roleid)
                    writenewsettings();
                    message.channel.send(`${lf.roleadded}: ${message.guild.roles.cache.get(roleid).name} (${roleid})`)
                    break;
                case "remove":
                    var roleid = roleid();
                    if (!v.beepBot.settings[guildid].adminroles.includes(roleid)) return message.channel.send(lf.rolenotincluded)
                    v.beepBot.settings[guildid].adminroles = v.beepBot.settings[guildid].adminroles.filter(f => f !== roleid)
                    writenewsettings();
                    message.channel.send(`${lf.roleremoved}: ${message.guild.roles.cache.get(roleid).name} (${roleid})`)
                    break;
                case "removeall":
                    if (v.beepBot.settings[guildid].adminroles.length === 0) return message.channel.send(lf.rolearrayempty)
                    ...
                    collector.on('collect', message => {
                        if (message.content == "y") {
                            v.beepBot.settings[guildid].adminroles = []
                            writenewsettings(); 
        ...
        case "modrole":
            ...
            switch(args[1].toLowerCase()) {
                case "add":
                    var roleid = roleid();
                    if (v.beepBot.settings[guildid].moderatorroles.includes(roleid)) return message.channel.send(lf.rolealreadyadded)
                    v.beepBot.settings[guildid].moderatorroles.push(roleid)
                    writenewsettings();
                    message.channel.send(`${lf.roleadded}: ${message.guild.roles.cache.get(roleid).name} (${roleid})`)
                    break;
                case "remove":
                    var roleid = roleid();
                    if (!v.beepBot.settings[guildid].moderatorroles.includes(roleid)) return message.channel.send(lf.rolenotincluded)
                    v.beepBot.settings[guildid].moderatorroles = v.beepBot.settings[guildid].moderatorroles.filter(f => f !== roleid)
                    writenewsettings();
                    message.channel.send(`${lf.roleremoved}: ${message.guild.roles.cache.get(roleid).name} (${roleid})`)
                    break;
                case "removeall":
                    if (v.beepBot.settings[guildid].moderatorroles.length === 0) return message.channel.send(lf.rolearrayempty)
                    const collector = new v.Discord.MessageCollector(message.channel, m => m.author.id === message.author.id, {time: 10000});
                    message.channel.send(lang.general.areyousure)
                    collector.on('collect', message => {
                        if (message.content == "y") {
                            v.beepBot.settings[guildid].moderatorroles = []
                            writenewsettings(); 
                            message.channel.send(lf.rolearraycleared) } });
                    break;
                default:
        ...
        case "systemchannel":
            ...
            switch(args[1].toLowerCase()) {
                case "set":
                    ...
                    v.beepBot.settings[guildid].systemchannel = channelid;
                    writenewsettings();
                    message.channel.send(`${lf.systemchannelset}: ${message.guild.channels.cache.get(channelid).name} (${channelid})`)
                    break;
                case "remove":
                    if (v.beepBot.settings[guildid].systemchannel === null) return message.channel.send(lf.systemchannelnotset)
                    v.beepBot.settings[guildid].systemchannel = null
                    writenewsettings();
        ...
        case "greetmessage":
        case "greetmsg":
            if (v.beepBot.settings[guildid].systemchannel == null) return message.channel.send(lf.greetmsgsystemchannelnotset)
            ...
            switch(args[1].toLowerCase()) {
                case "set":
                    ...
                    v.beepBot.settings[guildid].greetmsg = String(args.join(" ")); //join everything together
                    writenewsettings();
                    message.channel.send(lf.msgset)
                    break;
                case "remove":
                    if (v.beepBot.settings[guildid].greetmsg === null) return message.channel.send(lf.msgnotset)
                    v.beepBot.settings[guildid].greetmsg = null
                    writenewsettings();
        ...
        case "byemsg":
            ...
            switch(args[1].toLowerCase()) {
                case "set":
                    ...
                    v.beepBot.settings[guildid].byemsg = String(args.join(" ")); //join everything together
                    writenewsettings();
                    message.channel.send(lf.msgset)
                    break;
                case "remove":
                    if (v.beepBot.settings[guildid].byemsg === null) return message.channel.send(lf.msgnotset)
                    v.beepBot.settings[guildid].byemsg = null
                    writenewsettings();
                    message.channel.send(lf.msgremoved)
                    break;
                default:
        ...
        case "joinroles":
            ...
            switch(args[1].toLowerCase()) {
                case "add":
                    var roleid = roleid();
                    if (v.beepBot.settings[guildid].memberaddroles.includes(roleid)) return message.channel.send(lf.rolealreadyadded)
                    v.beepBot.settings[guildid].memberaddroles.push(roleid)
                    writenewsettings();
                    message.channel.send(`${lf.roleadded}: ${message.guild.roles.cache.get(roleid).name} (${roleid})`)
                    break;
                case "remove":
                    var roleid = roleid();
                    if (!v.beepBot.settings[guildid].memberaddroles.includes(roleid)) return message.channel.send(lf.rolenotincluded)
                    v.beepBot.settings[guildid].memberaddroles = v.beepBot.settings[guildid].memberaddroles.filter(f => f !== roleid)
                    writenewsettings();
                    message.channel.send(`${lf.roleremoved}: ${message.guild.roles.cache.get(roleid).name} (${roleid})`)
                    break;
                case "removeall":
                    if (v.beepBot.settings[guildid].memberaddroles.length === 0) return message.channel.send(lf.rolearrayempty)
                    const collector = new v.Discord.MessageCollector(message.channel, m => m.author.id === message.author.id, {time: 10000});
                    message.channel.send(lang.general.areyousure)
                    collector.on('collect', message => {
                        if (message.content == "y") {
                            v.beepBot.settings[guildid].memberaddroles = []
                            writenewsettings(); 
                            message.channel.send(lf.rolearraycleared) } });
                    break;
                default:
        ...
        default:
            message.channel.send({embed:{
                ...
                fields: [{ 
                        name: "Prefix:", 
                        value: `\`${v.beepBot.settings[guildid].prefix}\``, 
                        inline: true }, 
                    { 
                ...
                footer: { icon_url: message.author.displayAvatarURL(), text: `${lang.general.requestedby} ${message.author.username} • ${lang.cmd.help.help}: ${v.beepBot.settings[guildid].prefix}settings help` }
            } })
            return; }
}
